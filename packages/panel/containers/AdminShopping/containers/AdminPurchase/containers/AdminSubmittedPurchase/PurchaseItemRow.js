...
export default function PurchaseItemRow({
  ...
  const { id, title, unit, price, reason, supplied_amount } = item;
  const totalreceivedItems =
    reception?.[id]?.reduce((sum, item) => sum + parseFloat(item.amount), 0) ||
    0;
  const isMock = !id;
      ...
      <TableRow className={loading ? "u-pointer-events-none" : ""}>
        ...
        >
          {isMock ? (
            ...
            <div className="d-flex justify-content-between align-items-center w-100 p-2">
              <div>{englishNumberToPersianNumber(supplied_amount)}</div>
        ...
        >
          {isMock ? (
            ...
            <div className="d-flex justify-content-between align-items-center w-100 p-2">
              <div>{englishNumberToPersianNumber(totalreceivedItems)}</div>
              <div className="text-nowrap mr-2" color="text.quaternary">
                {unitText}
              </div>
            </div>
        ...
        >
          {isMock ? (
            ...
            <div className="d-flex justify-content-between align-items-center w-100 p-2">
              <div>{priceFormatter(price * totalreceivedItems || 0)}</div>
              <div
                className="text-nowrap mr-2"
                style={{ color: theme.palette.text.quaternary }}
              >
            ...
            >
              <Button
                onClick={() => {
                  ...
                  addReception(parseFloat(receptionAmount), modal);
                  setModal(null);
                }}
                color="primary"
                variant="contained"
